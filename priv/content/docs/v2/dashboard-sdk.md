...
```text
http://localhost:4177/tools/dashboard-wasm-harness/?manifest=/wifi-dashboard/dist/manifest.json&wasm=/wifi-dashboard/dist/renderer.js&frames=/wifi-dashboard/dist/sample-frames.json&settings=/wifi-dashboard/dist/sample-settings.json
```
...
- OpenSpec change tracking the SDK surface:
  ...
  `openspec/changes/update-wifi-dashboard-react-shell` in
  `~/src/serviceradar`.
